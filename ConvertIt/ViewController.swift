...
//  ConvertIt
//
//  Created by Ellana Lawrence on 9/29/19.
...
class ViewController: UIViewController {
        ...
        guard let inputValue = Double(userInput.text!) else {
            if userInput.text != ""{
             ...
             return
                ...
                default:
                    showAlert(title: "Unexpected Error", message: "Contact the developer and share that conversion \"\(conversionString)\" could not be identified.")
                }
    ...
    @IBAction func signSegmentSelected(_ sender: UISegmentedControl) {
        if signSegment.selectedSegmentIndex == 0 { // + selected, remove any -
            userInput.text?.replacingOccurrences(of: "-", with: "")
        } else {
